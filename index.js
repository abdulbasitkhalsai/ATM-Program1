...
if (pinAnswer.pin === myPin) {
    ...
    if (transType.transType === "Balance Inquiry") {
        ...
        if (transAmount.amountWithdrawl <= myBalance && !(transAmount.amountWithdrawl % 500)) {
            ...
            console.log("Please enter the amount multiplied by 500");
        }
        ;
    }
        ...
        {
            if (updatedPin.newPin >= 1000 && updatedPin.newPin <= 9999) {
                console.log(chalk.green.bold("Pin Updated Successfully"));
                myPin = updatedPin.newPin;
            }
            else
                console.log(chalk.red.bold("Pin should be 4 digit number, Please try again"));
        }
        ;
    }
